 ...
 */
	...
	* Kąt \e roll reprezentuje rotację zgodnie z ruchem wskazówek zegara
       ...
       double _Ang_Roll_deg = 0;
         ...
         /*!
        * \brief Pozycja lokalnego układu współrzędnych.
	*
	* Pozycja lokalnego układu współrzędnych. Przyjmuje się,
	...
	* że współrzędne wyrażone są w metrach.
       ...
       Vector3D  _Position_m;
        ...
        /*!
        * \brief Współczynnik skali względem wymiaru \e OX, \e OY, \e OZ.
        */
       Vector3D _Scale;

       /*!
        * \brief Wartości kolorów RGB.
        */
	...
	* Udostępnia nazwę obiektu w trybie tylko do odczytu.
       ...
       const std::string & GetName() const { return _Name; }
        ...
        /*!
       * \brief Udostępnia współrzędne położenia środka układu lokalnego w trybie tylko do odczytu.
       *
       * Udostępnia współrzędne punktu, który definiuje położenia środka układu lokalnego
       * w trybie tylko do odczytu.
       * Domyślnie przyjmuje się, że jest to geometryczny środek bryły.
       */
       const Vector3D & GetShift_m() const { return _Shift_m; }


        /*!
       * \brief Zmienia współrzędne położenia środka układu lokalnego.
       *
       * Zmienia współrzędne punktu, który definiuje śodek lokalnego ukłądu.
       * Domyślnie przyjmuje się, że jest to geometryczny środek bryły.
       * \param[in] rPos_m - współrzędne nowgo położenia. Przyjmuje się,
       *                     że są one wyrażone w metrach.
       */
       void SetShift_m(const Vector3D &rPos_m) { _Shift_m = rPos_m; }

        /*!
       * \brief Udostępnia wartości wektora skalowania
       */
       const Vector3D & GetScale() const { return _Scale; }


        /*!
       * \brief Zmienia wartości wektora skali
       * \param[in] rScale - nowe wartości dla skali
       */
       void SetScale(const Vector3D &rScale) { _Scale = rScale; }

       /*!
       * \brief Zmienia wartości koloru obiektu
       * \param[in] rColors - nowe wartości wartości RGB
       */
       void SetRGB(const Vector3D &rColors) { for(int i=0; i<3; i++){RGB[i] = rColors[i];}; }

        /*!
       * \brief Zwraca wartość składowej koloru obiektu
       * \param[in] elem - numer składowej koloru RGB
       */
       int GetRGB(int elem){
               if (elem <3){
                       return RGB[elem];
       ...
       void GetStateDesc(std::string * OutMsg){
                std::string temp = "Name="+_Name+" ";
                char buffor[50];

                sprintf(buffor, "RGB=(%d,%d,%d) ", RGB[0], RGB[1], RGB[2]);
                temp+=buffor;

                sprintf(buffor, "Scale=(%.2f,%.2f,%.2f) ", _Scale[0], _Scale[1], _Scale[2]);
                temp+=buffor;

                sprintf(buffor, "Shift=(%.2f,%.2f,%.2f) ", _Shift_m[0], _Shift_m[1], _Shift_m[2]);
                temp+=buffor;

                sprintf(buffor, "RotXYZ_deg=(%.2f,%.2f,%.2f) ", _Ang_Roll_deg, _Ang_Pitch_deg, _Ang_Yaw_deg);
                temp+=buffor;
                
                sprintf(buffor, "Trans_m=(%.2f,%.2f,%.2f) \n",  _Position_m[0], _Position_m[1], _Position_m[2]);
                temp+=buffor;
